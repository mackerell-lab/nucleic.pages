...
const report={repositoryOverride: pinnedRepository, loadedSourceSha256: {}, servedSourceSha256: {}, started:new Date().toISOString(),phases:[],errors:[],sourceSha256:{},limitations:'Single instrumented shared-host run; wall intervals inclusive and overlapping; CPU sampling approximate, no universal performance claim.'};
...
if (pinnedBody !== null) report.sourceSha256['core/repository.js'] = sha256(pinnedBody);
...
await page.route(url => url.origin === sourceBase.origin && url.pathname.startsWith(sourceBase.pathname) && url.pathname.endsWith('.js'), async route => {
 ...
 let body = file === 'core/repository.js' && pinnedBody !== null ? pinnedBody : await response.text();
