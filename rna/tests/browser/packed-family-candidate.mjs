...
import { configurePackedFamilyCandidate } from './packed-family-candidate-routing.mjs';
...
const sourceRelative = 'nucleic.pages/assets/pure_rna/releases/full_packed_coordinate_20260919/manifest.json';
const candidateUrl = new URL(process.env.RNA_FAMILY_CANDIDATE_URL
  || '/data/pure_rna/packed_family_candidate_20260919/candidate.json', base).href;
...
try {
  ...
  const routing = await configurePackedFamilyCandidate(page, candidateUrl, {
  ...
  const evidence = await page.evaluate(async ({ routing }) => {
    ...
    const source = new RnaDataRepository({ manifestUrl: routing.originalManifestUrl, maxCachedFamilies: 1,
      fetchImpl: (input, options) => {
        ...
        if (url.pathname.includes('/families/bundles/')) url.searchParams.set('rna-family-source', '1');
        return fetch(url.href, options);
      } });
    const manifest = await repository.loadManifest(), original = await source.loadManifest();
    if (!original.family_bundles) throw new Error('Expected unchanged source family bundles');
    if (original.families.some(item => item.encoding === 'rna-family-float64-1')) throw new Error('Independent source unexpectedly uses packed family transport');
    ...
    if (familyRequests.length || bundleRequests.length) throw new Error('Manifest eagerly loaded family resources');
    for (const descriptor of manifest.families) {
      ...
      if (familyRequests.length - before.families !== 1 || familyRequests.at(-1) !== descriptor.path) throw new Error('Family load fetched unrelated family payloads');
      const allowedBundles = new Set(routing.familyBundles[descriptor.id].map(reference =>
        new URL(`families/bundles/${reference}.json.gz`, routing.sourceManifestUrl).href));
      if (bundleRequests.slice(before.bundles).some(url => !allowedBundles.has(url))) throw new Error('Family load fetched unrelated bundles');
      if (families.length === 0 && bundleRequests.length !== allowedBundles.size) throw new Error('First family omitted or repeated bundle requests');
    ...
    const payloadRetry = new RnaDataRepository({ manifestUrl: routing.sourceManifestUrl, maxCachedFamilies: 1, fetchImpl: (input, options) => {
      if (String(input) === manifest.families[0].path && ++payloadAttempts === 1) return Promise.resolve(new Response('Intentional packed family payload retry', { status: 503 }));
      return fetch(input, options);
    } });
    let payloadFailure = null;
    try { await payloadRetry.loadFamily(first); } catch (error) { payloadFailure = error.message; }
    if (!payloadFailure?.includes('503') || payloadRetry.promises.has(`family:${first}`)) throw new Error('Packed family payload failure did not evict rejected promise');
    const payloadRecovered = await payloadRetry.loadFamily(first);
    equal(payloadRecovered.rows, expected.rows, 'payload retry rows');
    if (payloadAttempts !== 2) throw new Error('Packed family payload retry attempts differ');
    return { families, totalRows, exactAllFieldEquality: true, independentSourceManifest: routing.originalManifestUrl,
      ...
      warmMs, warmAdditionalRequests: 0, exactFamilyRequestScope: true,
      retry: { failureInjected, failedMessage, failedBundleAttempts, exactAllFieldEquality: true },
      payloadRetry: { failure: payloadFailure, attempts: payloadAttempts, exactAllFieldEquality: true },
