...
export async function configurePackedFamilyCandidate(page, candidateUrl, { sourceManifestUrl, sourceManifestPath } = {}) {
  ...
  assert.equal(candidate.schema_version, 'rna-family-packed-candidate-1');
  ...
  assert.deepEqual(candidate.family_bundles, source.family_bundles, 'Packed family candidate changed shared bundle registry');
  ...
  for (const descriptor of candidate.families) {
    ...
    assert.equal(descriptor.encoding, 'rna-family-float64-1', 'Candidate family transport was not packed');
  ...
  for (const [href, descriptor] of resources) {
    ...
    if (descriptor.id) {
      ...
      assert.equal(payload.encoding, 'rna-family-float64-1');
      assert.equal(payload.build_id, candidate.build_id);
      familyBundles[descriptor.id] = [...new Set(Object.values(payload.columns)
        .filter(column => record(column) && Object.hasOwn(column, 'bundle')).map(column => column.bundle))];
      for (const reference of familyBundles[descriptor.id]) assert(Object.hasOwn(candidate.family_bundles, reference));
    }
  ...
  return { candidateUrl: url.href, sourceManifestUrl: sourceUrl.href,
    ...
    verifiedResources: resources.size, verifiedBytes, familyBundles };
}
