...
const assetsRoot = path.join(workspace, 'nucleic.pages/assets/pure_rna');
const pointer = JSON.parse(await readFile(path.join(assetsRoot, 'manifest.json'), 'utf8'));
const releaseManifest = path.resolve(assetsRoot, pointer.manifest || `releases/${pointer.build_id}/manifest.json`);
if (!releaseManifest.startsWith(`${path.resolve(assetsRoot)}${path.sep}`)) throw new Error('RNA active manifest escapes the asset root');
const release = path.dirname(releaseManifest);
...
const familyPayload = await read(family.path);
const table = deepFreeze(familyPayload?.encoding === FAMILY_COLUMNAR_ENCODING ? decodeFamilyRows(familyPayload) : familyPayload);
